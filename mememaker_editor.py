...
class mememaker_editor() :
    ...
    ###################################
    def redefine_predits() :
        global preditbw
        global preditcol
        global usable_edit_path
        preditbw = cv2.imread(usable_edit_path, 0)
        preditcol = cv2.imread(usable_edit_path, 1)
    ...
    def gt(panel):
        global preditbw
        edit = cv2.adaptiveThreshold(preditbw, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 21, 2)
        cv2.imwrite('usable_edit.jpg', edit)
        edit = ImageTk.PhotoImage(Image.open(mememaker_gui.usable_edit_path))
        panel.config(image = edit)
        panel.photo_ref = edit
   
    def bw(panel):
    	cv2.imwrite('editted.jpg', predit)
